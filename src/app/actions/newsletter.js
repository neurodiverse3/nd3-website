...
const newsletterLimits = new Map(); // key: ip, value: array of timestamps (numbers)
...
function checkNewsletterRateLimit(ip) {
  ...
  let timestamps = newsletterLimits.get(ip) || [];
  ...
  timestamps.push(now);
  newsletterLimits.set(ip, timestamps);
  return true; // Allowed
}
...
export async function subscribeNewsletter(prevState, formData) {
  ...
  if (honey) {
    console.log("🤖 Spam bot caught in newsletter honeypot!");
    return { success: true, message: "You’re in." };
  }

  // 2. IP-based Rate limiting check
  const clientHeaders = await headers();
  const forwarded = clientHeaders.get('x-forwarded-for');
  const ip = forwarded ? forwarded.split(',')[0].trim() : '127.0.0.1';

  if (!checkNewsletterRateLimit(ip)) {
    return { success: false, error: "Too many subscription attempts. Please try again in a few minutes." };
  }
