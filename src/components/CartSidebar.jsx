    ...
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-bg-primary h-full border-l border-border-rule p-8 md:p-10 shadow-[0_0_120px_rgba(0,0,0,1)] flex flex-col animate-in slide-in-from-right duration-500 overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <span className="text-xs md:text-sm font-mono font-bold uppercase tracking-widest text-accent block">n³ BASKET</span>
          ...
          {isCheckingOut ? (
            <div className="flex flex-col items-center justify-center py-24 text-center space-y-6">
              ...
              <div className="space-y-2">
                <span className="text-xs md:text-sm font-mono font-bold uppercase tracking-widest text-accent block">SECURE CONNECTION</span>
              ...
              <button 
                ...
                className="px-4 py-2 mt-4 bg-transparent text-text-muted hover:text-accent border border-border-rule hover:border-accent text-xs md:text-sm font-mono uppercase tracking-widest transition-all cursor-pointer"
              >
                Cancel
              </button>
            </div>
            ...
            <>
              ...
              {cart.length > 0 && (
                <div className="space-y-4">
                  <span className="text-xs md:text-sm font-mono font-bold text-text-muted uppercase tracking-widest block">SELECTED ITEMS</span>
                  {cart.map(item => (
                    <div key={item.id} className="p-4 bg-bg-primary border border-border-rule flex justify-between items-center group shadow-md relative">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-xs md:text-sm text-text-muted font-black tracking-widest mt-1.5 uppercase font-mono">DIGITAL DOWNLOAD</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
              ...
              {cart.length === 0 && securedDownloads.length === 0 && (
                <div className="text-center py-28 opacity-30 text-text-muted">
                  ...
                  <p className="font-black uppercase text-xs md:text-sm tracking-[0.25em]">YOUR BASKET IS EMPTY</p>
                </div>
              )}
              ...
              {securedDownloads.length > 0 && (
                <div className="space-y-4 pt-6 border-t border-border-rule/50">
                  <span className="text-xs md:text-sm font-mono font-bold text-green-400 uppercase tracking-widest block">SECURED RESOURCES ({securedDownloads.length})</span>
                      ...
                      return (
                        <div key={item.id} className="p-4 bg-green-500/5 border border-green-600/30 flex justify-between items-center shadow-sm">
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-none bg-gradient-to-br ${item.color} flex items-center justify-center font-black text-xs md:text-sm text-white shrink-0 border border-green-600/20`}>
                              PDF
                            </div>
                            <div className="text-left">
                              <h4 className="font-black text-xs uppercase tracking-tight text-fg-primary leading-tight">{item.title}</h4>
                              <p className="text-xs md:text-sm text-text-muted font-bold mt-1 uppercase font-mono">License Verified ✓</p>
                            </div>
                          </div>
                          <button 
                            onClick={() => handleCompileDownload(item)}
                            disabled={downloadingId !== null}
                            className="flex items-center justify-center gap-1.5 bg-green-500/10 hover:bg-green-500/20 border border-green-500/40 text-green-400 px-3 py-1.5 text-xs md:text-sm font-black uppercase tracking-wider cursor-pointer disabled:opacity-50 transition-all"
                          >
                            {isThisDownloading ? (
                              <>
        ...
        {cart.length > 0 && !isCheckingOut && (
          <div className="mt-6 pt-6 border-t border-border-rule space-y-5">
            {checkoutError && (
              <div className="bg-bg-primary border-2 border-accent p-3 shadow-[2px_2px_0px_var(--accent)] text-left flex flex-col gap-1 animate-in fade-in slide-in-from-bottom-2 duration-200">
                <div className="flex justify-between items-center w-full">
                  <span className="text-xs md:text-sm font-black uppercase text-accent tracking-widest">⚠️ CHECKOUT WARNING</span>
                  <button onClick={() => setCheckoutError(null)} className="text-text-muted hover:text-fg-primary text-xs md:text-sm uppercase font-bold tracking-widest font-mono cursor-pointer">[DISMISS]</button>
                </div>
                <p className="text-xs md:text-sm text-text-muted leading-relaxed font-mono">
                  {checkoutError}
                </p>
              </div>
            )}
            ...
            <div className="flex justify-between items-end">
              <span className="text-text-muted uppercase text-xs md:text-sm font-black tracking-[0.2em]">Investment Total</span>
