...
export const TableOfContents = ({ headings = [], isMobile = false, pillar = '' }) => {
  ...
  // Custom styles for active outline link
  const activeStyles = {
    unmasked: { color: 'var(--pillar-label-unmasked, #FF2D87)', borderColor: 'var(--pillar-label-unmasked, #FF2D87)' },
    tools: { color: 'var(--pillar-label-tools, #FFAC1C)', borderColor: 'var(--pillar-label-tools, #FFAC1C)' },
    digital: { color: 'var(--pillar-label-digital, #00F0FF)', borderColor: 'var(--pillar-label-digital, #00F0FF)' }
  };
  
  const activeStyle = activeStyles[pillarKey] || activeStyles.unmasked;
  ...
  const accentColor = accentBorderColors[pillarKey] || accentBorderColors.unmasked;
          ...
          <nav className="mt-4 pt-4 border-t border-border-rule/60 flex flex-col gap-2.5 animate-in slide-in-from-top-2 duration-200">
            {headings.map((h) => (
              <a
                ...
                style={activeId === h.id ? activeStyle : undefined}
              >
                <span>{h.text}</span>
              </a>
            ))}
  ...
  return (
    <nav className="hidden xl:flex flex-col gap-4 w-full select-none text-left font-sans sidebar-card p-5 relative overflow-hidden">
      {/* Dynamic top accent border */}
      <div className={`absolute top-0 left-0 right-0 h-[3px] ${accentColor}`} />
      
      <div className="text-xs font-mono font-bold tracking-widest text-text-muted uppercase mb-1 border-b border-border-rule/85 pb-2 pt-1">
        Outline
      </div>
